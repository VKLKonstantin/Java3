 ...
 * с аннотациями @Test, а по завершении всех тестов – метод с аннотацией @AfterSuite. К каждому тесту необходимо добавить
 * приоритеты (int числа от 1 до 10), в соответствии с которыми будет выбираться порядок их выполнения. Если приоритет одинаковый,
...
public class Main {
    ...
    public static void main(String[] args) {
        Class testMethods = TestMethods.class;
        try {
            ClassTest.start(testMethods);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
